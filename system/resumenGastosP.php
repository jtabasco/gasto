...
<div class="modal shadow" id="prestamo" tabindex="-1" aria-labelledby="aPrestamo" aria-hidden="true">
  <div class="modal-mb modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <div class="row">
          ...
          <div class="col-12">
			...
			<?php
				$consulta = $conn->query("select * from usuarios where activo='si' and id not in (".$_SESSION['idUser'].")");
...
$(document).ready(function(){
		$('#compra','#prestamo').modal({backdrop:'static'}); //para que no se cierre con click fuera
...
// boton aceptar prestamo
	$('#AceptarPrestamo').click(function(){
			// chequemos que parametros son obligatorios
			if ($('#aDetallePrestamo').val().length<=0||$('#aMontoPrestamo').val().length<=0||$('#aFechaPrestamo').val().length<=0||$('#adeudorPrestamo').val()==0){
					toastr.options = {"positionClass": "toast-bottom-right","preventDuplicates": true};
				 toastr.error(" COMPLETE DATOS OBLIGATORIOS", "");
				...
				return;
							        ...
							        $('#spinPrestamo').removeAttr('hidden');
							        $('#spinPrestamo').show();
			   			   			let parametros = {
												...
												$.ajax({
													...
													success: function(resp){
														...
														toastr.options = {"positionClass": "toast-bottom-right","preventDuplicates": true};
														 toastr.success("!!!PRESTAMO REGISTRADO!!! ","");
														 // actualizamos el subtotal de por cobrar
														 $.ajax({url: 'subtotalPorCobrar.php',type:'POST',
			   													   success: function(resp){
																   			   	//actualizo el control
...
// Accion de añadir deudores
	...
	// Accion de añadir deudores
	// $('#adeudorPrestamo').on('change', function() {
	// //$('#Anadir').click(function(){
	// 		// chequemos que parametros son obligatorios
	// 		if ($('#adeudorPrestamo').val().length<=0){
	// 			 toastr.options = {"positionClass": "toast-bottom-right","preventDuplicates": true};
	// 			 toastr.error("COMPLETE DATOS", "");
