...
$sql = "SELECT sum(Monto) as importe FROM `Gastos` where month(fecha)='$mes' and year(fecha)='$ano' GROUP by year(fecha),month(fecha)";
...
$sql = "SELECT categoria.categoria, sum(Monto) as importe FROM `Gastos` inner join categoria where categoria.id=Gastos.idcat and year(fecha)='$ano' and month(fecha)='$mes' GROUP by year(fecha),month(fecha),categoria";
// Ejeuctar el SQL
$query = $conn->query($sql);
// Recorrer los resultados
while($data = $query->fetch_object()){
